...
export interface TranscriptExcerpt {
  ...
  occurrence: number;
  /**
   * When it was said, from the caption track's own cue timings.
   *
   * Absent rather than guessed when the platform published no timings — the
   * speech-to-text fallback returns words with no clock, and a moment
   * estimated from how far through the string the match sits is wrong by
   * however long the speaker paused. A row that shows no timecode is honest;
   * one that shows a plausible wrong number is not.
   */
  startMs?: number;
  /** The same instant as "4:12", for a person to read and a link to jump to. */
  timecode?: string;
}

/** A caption cue as get_post_transcript returns it: a time and where it lands. */
interface TranscriptCue {
  startMs: number;
  endMs?: number;
  /** Offset of this cue's words in the transcript, in UTF-16 code units. */
  offset: number;
}

/** "4:12", or "1:04:12" once there is an hour to show. */
...
function cueAt(cues: TranscriptCue[], position: number): TranscriptCue | null {
  ...
  for (const cue of cues) {
    if (cue.offset > position) break;
    found = cue;
  }
  return found;
}

/** Attach the moment each excerpt was said, where the track carried one. */
export function timeExcerpts(excerpts: TranscriptExcerpt[], rawCues: unknown): TranscriptExcerpt[] {
  const cues = (Array.isArray(rawCues) ? rawCues : [])
    .filter((c): c is TranscriptCue =>
      !!c && typeof c === "object"
      && typeof (c as TranscriptCue).offset === "number"
      && typeof (c as TranscriptCue).startMs === "number")
    .sort((a, b) => a.offset - b.offset);
  if (!cues.length) return excerpts;
  return excerpts.map((ex) => {
    const cue = cueAt(cues, ex.position);
    if (!cue) return ex;
    return { ...ex, startMs: cue.startMs, timecode: formatTimecode(cue.startMs) };
  });
}
        ...
        try {
          ...
          if (matchCount > 0) {
            hits.push({
              ...
              postId: id,
              matchCount,
              // TikTok and YouTube both publish a timed caption track, and
              // those are the only two platforms this tool narrows to — so a
              // hit here almost always knows the moment it was said. The
              // speech-to-text fallback carries no clock, and an excerpt from
              // one simply arrives without a timecode.
              excerpts: timeExcerpts(excerpts, structured.cues),
