...
(function(){
  ...
  document.addEventListener("click",function(e){
    ...
    spokenExcerpts().forEach(function(x){byId[x.id]=x.text;});
  ...
  function postCard(p,wide,pickable,hideActions){
    ...
    if(p.mediaItems&&p.mediaItems.length){
      for(var i=0;i<p.mediaItems.length;i++){
        ...
        // "proxy_url" is the same asset served through Nooticr — the raw
  ...
  function renderProduct(d){
    ...
    return '<div class="fade-in"><div class="card card-wide"><div class="card-body">'
      ...
      +"</div>"
      // One button, not the two the design drew: connect_social_account is a
      // real tool and analyze_product is not one this server serves, so the
      // playbook is stated as a fact rather than offered as a button that
      // would fail.
  ...
  document.addEventListener("click",function(e){
    ...
    if(l)ownActionButton(l,"list_social_connections",{},"Loading…");
  });
