...
`nooticr-mcp` is an MCP server that reads TikTok / Instagram / YouTube / X /
...
over. It runs no model of its own. Authenticated with the user's nooticr
account (OAuth) and billed against the user's nooticr server credits, at the
price of the fetch.
...
| `NOOTICR_API_KEY` | `nk_…` | optional API key from `api-key create` — no browser, no expiry, revoked by id; the path for server-side deployments |
